        ...
        if request.method == "GET":
            for x in review.papers:
                if not x.get('decisions') or len(x.get('decisions')) < MIN_DECISIONS:
                    return render(request, 'literature_review/screen_paper.html', {'review': review,
                                                                          'paper': x})
            ...
            eligibility_decision = make_decision(exclusions=exclusions, inclusions=inclusions)
            ...
            edited_index = [index_i for index_i, x in enumerate(review.papers) if x['id'] == paper_id][0]
